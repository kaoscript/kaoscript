namespace Generator {
	export {
		...
		func buildHelper(fragments: SourceGeneration.MarkWriter, args: String, hasDeferred: Boolean, node: AbstractNode): FragmentHelper { # {{{
	...
	func getTester(this, hasDeferred: Boolean, fragments: SourceGeneration.MarkWriter, node: AbstractNode, type: Type): String { # {{{
		...
		if !isTest {
			...
			if isUsingTestings(branch, startIndex) {
				if nIndex + 1 == nLength && branch.order.length == 1 {
					...
					var mut useEachArgs = false
					var mut nullTested = false

					for var key, index in branch.order {
						useEachArgs = toTreeFragments(buildPath, args, tree, branch.columns[key], nullTested, helper, fragments, index, branch.order.length, Junction.NONE, alternative || hasAlternative(branch, index), startIndex, min - branch.min, node)
				...
				var useEachArgs = toTreeFragments(buildPath, args, tree, branch.columns[branch.order[0]], false, helper, fragments, 0, 1, Junction.NONE, alternative || hasAlternative(branch, 0), startIndex, min - branch.min, node)
