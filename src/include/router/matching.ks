namespace Matching {
	...
	func mergeResults(results: CallMatchResult[]): CallMatchResult? { # {{{
		...
		if results.every((result, _, _) => result is PreciseCallMatchResult) {
			...
			if precise {
				return new PreciseCallMatchResult([match for var match of perFunctions])
			}
		}
					...
					if cursor.argument.isUnion() {
						...
						for var type in cursor.argument.discardAlias().types() {
							...
							for var key in tree.order while nf {
								if matchTreeNode(tree, tree.columns[key], duplicateCursor(cursor, type), new Matches(), newContext) {
									nf = false
								}
							}
		...
		func matchIndex(
			...
			if indexeds.length == 0 {
				...
				if preciseFunctions.length == possibleFunctions.length {
					...
					return new PreciseCallMatchResult(matches: [match])
				}
				else {
					var possibilities = []
					...
					return new LenientCallMatchResult(possibilities, positions, labels)
				}
			}
			else {
