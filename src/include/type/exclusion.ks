class ExclusionType extends Type {
	...
	override toBlindTestFragments(funcname, varname, casting, generics, subtypes, junction, fragments, node) { # {{{
		...
		if @types[0].isAny() {
			@types[1].toBlindTestFragments(funcname, varname, casting, generics, subtypes, Junction.AND, fragments.code('!'), node)

			for var type in @types from 2 {
				fragments.code(' && ')

				type.toBlindTestFragments(funcname, varname, casting, generics, subtypes, Junction.AND, fragments.code('!'), node)
			}
		}
		else {
			@types[0].toBlindTestFragments(funcname, varname, casting, generics, subtypes, Junction.AND, fragments.code('!'), node)
