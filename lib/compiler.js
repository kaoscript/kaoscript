...
module.exports = function() {
	...
	var NotImplementedException = Helper.class({
		...
		__ks_cons: function(args) {
			if(args.length >= 1 && arguments.length <= 3) {
				if(Type.isAny(args[0])) {
	...
	var $helper = {
		...
		methodCheckTree: function(methods, index, node, fragments, call, argName, refName, returns) {
			...
			for(var __ks_0 = 0, __ks_1 = methods.length, method; __ks_0 < __ks_1; ++__ks_0) {
				...
				var __ks_2 = $helper.methodTypes(method, index);
				for(var __ks_3 = 0, __ks_4 = __ks_2.length; __ks_3 < __ks_4; ++__ks_3) {
					type = __ks_2[__ks_3];
					nf = true;
					for(var __ks_5 = 0, __ks_6 = tree.length; nf && __ks_5 < __ks_6; ++__ks_5) {
						tt = tree[__ks_5];
						if(__ks_Array._im_contains(tt.type, type.type)) {
							item = tt;
							nf = false;
						}
					}
			...
			if(tree.length === 1) {
				...
				for(var __ks_5 = 0, __ks_6 = usages.length; __ks_5 < __ks_6; ++__ks_5) {
					usage = usages[__ks_5];
					var count = usage.types.length;
					for(var __ks_2 = 0, __ks_3 = usage.types.length; count >= 0 && __ks_2 < __ks_3; ++__ks_2) {
				...
				var ctrl = fragments.newControl();
				var ne = true;
				for(var i = 0, __ks_5 = tree.length; i < __ks_5; ++i) {
					item = tree[i];
					if((i + 1) === tree.length) {
						if(!ctrl.isFirstStep()) {
							ctrl.step().code("else");
