...
module.exports = function() {
	...
	var $type = {
		...
		type: function(data, scope, node) {
			...
			else if(__ks_0 === NodeKind.BinaryOperator) {
		...
		fromType: function(data, node) {
			...
			if(Type.isValue(data.typeName)) {
				if(data.typeName.kind === NodeKind.Identifier) {
					...
					var __ks_variable_1 = $variable.fromAST(data.typeName.object, node);
					if(__ks_variable_1 && (__ks_variable_1.kind === VariableKind.Variable) && __ks_variable_1.type && __ks_variable_1.type.properties) {
						var __ks_name_1 = data.typeName.property.name;
						var property = __ks_variable_1.type.properties[__ks_name_1];
						if(Type.isObject(property)) {
							property.accessPath = (__ks_variable_1.accessPath || __ks_variable_1.name.name) + ".";
							return property;
						}
					}
					else {
			...
			if(this._binary) {
				...
				if(this._dynamicRequirements.length) {
					...
					if(this._dynamicRequirements.length === 1) {
						...
						for(var __ks_0 = 0, __ks_1 = this._dynamicRequirements.length; __ks_0 < __ks_1; ++__ks_0) {
				...
				if(this._dynamicRequirements.length) {
					if(this._options.format.destructuring === "es5") {
						...
						fragments.push($code("\tvar ["));
						i = 0;
						for(var __ks_0 = this._dynamicRequirements.length, requirement; i < __ks_0; ++i) {
							requirement = this._dynamicRequirements[i];
							if(i !== 0) {
								fragments.push($comma);
							}
	...
	var $class = {
		...
		continuous: function(node, fragments) {
			...
			if(node._es5) {
				...
				var __ks_clazz_1 = fragments.newControl().code("class ", node._name);
				if(node._extends) {
					__ks_clazz_1.code(" extends ", node._extendsName);
				}
				__ks_clazz_1.step();
				var __ks_ctrl_1;
				if(!node._extends) {
					__ks_clazz_1.newControl().code("constructor()").step().line("this.__ks_init()").line("this.__ks_cons(arguments)").done();
				}
				if(noinit) {
					if(node._extends) {
						__ks_clazz_1.newControl().code("__ks_init()").step().line(node._extendsName + ".prototype.__ks_init.call(this)").done();
					}
					else {
						__ks_clazz_1.newControl().code("__ks_init()").step().done();
					}
				}
				else {
					++reflect.inits;
					__ks_ctrl_1 = __ks_clazz_1.newControl().code("__ks_init_1()").step();
					for(var name in node._instanceVariables) {
						var field = node._instanceVariables[name];
						if(Type.isValue(field.data.defaultValue)) {
							__ks_ctrl_1.newLine().code("this." + name + " = ").compile(field.defaultValue).done();
						}
					}
					__ks_ctrl_1.done();
					__ks_ctrl_1 = __ks_clazz_1.newControl().code("__ks_init()").step();
					if(node._extends) {
						__ks_ctrl_1.line(node._extendsName + ".prototype.__ks_init.call(this)");
					}
					__ks_ctrl_1.line(node._name + ".prototype.__ks_init_1.call(this)");
					__ks_ctrl_1.done();
				}
				for(var __ks_0 = 0, __ks_1 = node._constructors.length, method; __ks_0 < __ks_1; ++__ks_0) {
					method = node._constructors[__ks_0];
					$class.constructor(node, __ks_clazz_1, method.statement, method.signature, method.parameters, reflect);
				}
				$helper.constructor(node, __ks_clazz_1, reflect);
				if(Type.isValue(node._destructor)) {
					$class.destructor(node, __ks_clazz_1, node._destructor.statement, reflect);
					$helper.destructor(node, __ks_clazz_1, reflect);
				}
				for(var name in node._instanceMethods) {
					var methods = node._instanceMethods[name];
					for(var __ks_0 = 0, __ks_1 = methods.length, method; __ks_0 < __ks_1; ++__ks_0) {
						method = methods[__ks_0];
						$class.instanceMethod(node, __ks_clazz_1, method.statement, method.signature, method.parameters, reflect, name);
					}
					$helper.instanceMethod(node, __ks_clazz_1, reflect, name);
				}
				for(var name in node._abstractMethods) {
					var methods = node._abstractMethods[name];
					for(var __ks_0 = 0, __ks_1 = methods.length, method; __ks_0 < __ks_1; ++__ks_0) {
						method = methods[__ks_0];
						$class.abstractMethod(node, __ks_clazz_1, method.statement, method.signature, method.parameters, reflect, name);
					}
				}
				for(var name in node._classMethods) {
					var methods = node._classMethods[name];
					for(var __ks_0 = 0, __ks_1 = methods.length, method; __ks_0 < __ks_1; ++__ks_0) {
						method = methods[__ks_0];
						$class.classMethod(node, __ks_clazz_1, method.statement, method.signature, method.parameters, reflect, name);
					}
					$helper.classMethod(node, __ks_clazz_1, reflect, name);
				}
				__ks_clazz_1.done();
			}
		...
		sealed: function(node, fragments) {
			...
			if(node._es5) {
				...
				if(node._extends) {
					...
					__ks_ctrl_1 = __ks_clazz_1.newControl().code("constructor()").step();
					if(!noinit) {
						for(var name in node._instanceVariables) {
							var field = node._instanceVariables[name];
							if(Type.isValue(field.data.defaultValue)) {
					...
					__ks_ctrl_1.line("this.__ks_cons(arguments)");
					__ks_ctrl_1.done();
				}
				for(var __ks_0 = 0, __ks_1 = node._constructors.length, method; __ks_0 < __ks_1; ++__ks_0) {
					method = node._constructors[__ks_0];
		...
		methods: function(extend, node, fragments, header, methods, call, argName, refName, returns) {
			...
			if(methods.length === 0) {
				...
				if((method.min === 0) && (method.max >= Infinity)) {
					...
					var __ks_ctrl_1 = fragments.newControl();
					__ks_ctrl_1.code("if(" + argName + ".length >= " + method.min + " && " + argName + ".length <= " + method.max + ")").step();
					call(__ks_ctrl_1, method, 0);
					if(returns) {
						__ks_ctrl_1.done();
						fragments.line("throw new Error(\"Wrong number of arguments\")");
					}
					else {
						__ks_ctrl_1.step().code("else").step().line("throw new Error(\"Wrong number of arguments\")").done();
					}
				}
				else {
					call(fragments, method, 0);
				...
				for(var u = 0, __ks_3 = usages.length; u < __ks_3; ++u) {
					...
					if(usage.tree.length === usage.usage) {
						...
						if((u + 1) === usages.length) {
							...
							ctrl.code("if(");
							$helper.decide(node, ctrl, __ks_item_1.type, index, __ks_item_1.path, argName);
							ctrl.code(")");
						}
						ctrl.step();
						if(__ks_item_1.methods.length === 1) {
							call(ctrl, __ks_item_1.methods[0], __ks_item_1.methods[0].index);
						}
						else {
							$helper.methodCheckTree(__ks_item_1.methods, index + 1, node, ctrl, call, argName, refName, returns);
						}
					}
					else {
						$throw("Not Implemented", node);
	...
	var ClassDeclaration = Helper.class({
		...
		__ks_func_analyse_0: function() {
			...
			if(this._extends && !this._abstract && !$class.areAbstractMethodsImplemented(this._variable, this._extendsVariable, this._scope)) {
				$throw("Not implemented abstract methods for class '" + this._name + "' at line " + this._data.start.line, this);
			}
			if(this._sealed) {
	...
	var EnumDeclaration = Helper.class({
		...
		__ks_func_toStatementFragments_0: function(fragments, mode) {
			...
			if(this._new) {
				...
				for(var __ks_0 = 0, __ks_1 = this._members.length, member; __ks_0 < __ks_1; ++__ks_0) {
					member = this._members[__ks_0];
					member.toFragments(__ks_line_1 = fragments.newLine());
					__ks_line_1.done();
				}
			}
		},
	...
	var ExpressionStatement = Helper.class({
		...
		__ks_func_toFragments_0: function(fragments, mode) {
			...
			if(this._expression.isAssignable()) {
				...
				var __ks_line_1 = fragments.newLine();
				if(this._variable.length) {
					__ks_line_1.code($variable.scope(this));
				}
				__ks_line_1.compile(this._expression, Mode.None).done();
			}
	...
	var $function = {
		...
		useThisVariable: function(data) {
			...
			if(__ks_0 === NodeKind.ArrayExpression) {
				...
				if(Type.isValue(data.whenFalse) && data.$function.useThisVariable(data.whenFalse)) {
					return true;
				}
			}
			else if(__ks_0 === NodeKind.Literal) {
	...
	var ImplementDeclaration = Helper.class({
		...
		__ks_func_analyse_0: function() {
			...
			if(this._variable.kind === VariableKind.Class) {
				...
				$throw("Invalid class/variable for impl at line " + this._data.start.line, this);
			}
		},
		analyse: function() {
			if(arguments.length === 0) {
		...
		toKSFileFragments: function(node, fragments, data, metadata) {
			...
			if((importVarCount && importAll) || (importVarCount && importAlias.length) || (importAll && importAlias.length)) {
				...
				for(name in requirements) {
					...
					__ks_nf_1 = true;
					if(data.references) {
						for(var __ks_0 = 0, __ks_1 = data.references.length, reference; __ks_nf_1 && __ks_0 < __ks_1; ++__ks_0) {
							reference = data.references[__ks_0];
							if(Type.isValue(reference.foreign)) {
								if(reference.foreign.name === name) {
									if(__ks_first_1) {
			...
			if(importVarCount === 1) {
				...
				if(node._options.format.destructuring === "es5") {
					if(importCodeVariable) {
						...
						var __ks_line_1 = fragments.newLine().code("var ");
						var __ks_nf_1 = false;
						for(name in importVariables) {
							alias = importVariables[name];
							variable = exports[alias];
							if(variable.kind !== VariableKind.TypeAlias) {
								...
								__ks_line_1.code(alias + " = __ks__." + name);
							}
						}
						__ks_line_1.done();
					}
					...
					if(node._options.format.destructuring === "es5") {
						if(importCodeVariable) {
							...
							for(var i = 0, __ks_0 = variables.length; i < __ks_0; ++i) {
								...
								__ks_line_1.code(name + " = __ks__." + name);
							}
							__ks_line_1.done();
						}
					}
				...
				if(node._options.format.destructuring === "es5") {
					...
					for(alias in variables) {
						...
						if(variables[alias] === alias) {
							__ks_line_1.code(alias);
						}
						else {
							__ks_line_1.code(alias, ": ", variables[alias]);
						}
					}
					__ks_line_1.code("} = require(", $quote(moduleName), ")");
					__ks_line_1.done();
				}
			}
		}
	...
	var TryStatement = Helper.class({
		...
		__ks_func_toStatementFragments_0: function(fragments, mode) {
			...
			if(this._catchClauses.length) {
				...
				var __ks_error_1 = this._scope.acquireTempName();
				if(Type.isValue(this._data.catchClause.binding)) {
					ctrl.code("catch(", this._data.catchClause.binding.name, ")").step();
				}
				else {
					ctrl.code("catch(", __ks_error_1, ")").step();
				}
				if(Type.isValue(finalizer)) {
					ctrl.line(finalizer, "()");
				}
				ctrl.compile(this._catchClause);
				this._scope.releaseTempName(__ks_error_1);
			}
			else {
				var __ks_error_2 = this._scope.acquireTempName();
				ctrl.code("catch(", __ks_error_2, ")").step();
				if(Type.isValue(finalizer)) {
					ctrl.line(finalizer, "()");
				}
				this._scope.releaseTempName(__ks_error_2);
			}
			ctrl.done();
		},
		toStatementFragments: function() {
		...
		__ks_func_analyse_0: function() {
			if(this._data.name.kind === NodeKind.Identifier) {
				...
				if(this._scope.hasVariable(this._data.name.name, false)) {
					$throw("Already declared variable '" + this._data.name.name + "' at line " + this._data.name.start.line, this);
				}
	...
	var CreateExpression = Helper.class({
		...
		__ks_func_analyse_0: function() {
			...
			if((Type.isValue(__ks_0 = $variable.fromAST(this._data.class, this)) ? (variable = __ks_0, true) : false) && variable.abstract) {
				$throw("Can't instantiate class at line " + this._data.start.line, this);
			}
	...
	var CurryExpression = Helper.class({
		...
		__ks_func_toFragments_0: function(fragments, mode) {
			...
			if(this.isNullable() && !this._tested) {
				...
				this.module().flag("Helper");
				var __ks_kind_1 = this._data.scope.kind;
				if(__ks_kind_1 === ScopeKind.This) {
				...
				else if(__ks_kind_1 === ScopeKind.Null) {
