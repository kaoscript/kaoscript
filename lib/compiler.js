...
module.exports = function() {
	...
	var Exception = Helper.class({
		...
		__ks_func_captureStackTrace_0: function() {
			if(Type.isValue(Error.captureStackTrace)) {
	...
	var NotImplementedException = Helper.class({
		...
		__ks_cons_0: function() {
			...
			if(Type.is(arguments[++__ks_i], AbstractNode)) {
		...
		__ks_cons: function(args) {
			if(args.length === 1) {
				...
				if(Type.is(args[0], this.constructor.__ks_reflect.constructors[1].parameters[1].type)) {
					NotImplementedException.prototype.__ks_cons_1.apply(this, args);
				}
				else {
					if(Type.is(args[1], this.constructor.__ks_reflect.constructors[0].parameters[1].type)) {
						NotImplementedException.prototype.__ks_cons_0.apply(this, args);
					}
					else {
				...
				if(Type.is(args[1], this.constructor.__ks_reflect.constructors[1].parameters[1].type)) {
					NotImplementedException.prototype.__ks_cons_1.apply(this, args);
				}
				else {
					NotImplementedException.prototype.__ks_cons_2.apply(this, args);
	...
	var $runtime = {
		helper: function(node) {
			...
			Type.isFunction(node.module) ? node.module().flag("Helper") : undefined;
		...
		type: function(node) {
			...
			Type.isFunction(node.module) ? node.module().flag("Type") : undefined;
	...
	var Attribute = Helper.class({
		...
		$static: {
			__ks_sttc_conditional_0: function(data, target) {
				...
				if(Type.isValue(data.attributes) ? data.attributes.length > 0 : false) {
					...
					for(var __ks_0 = 0, __ks_1 = data.attributes.length, attr; __ks_0 < __ks_1; ++__ks_0) {
						...
						var __ks_2;
						if((attr.declaration.kind === NodeKind.AttributeExpression) && (Type.isValue(__ks_2 = Attribute.get(attr.declaration, AttributeTarget.Conditional)) ? (attribute = __ks_2, true) : false)) {
			...
			__ks_sttc_configure_0: function(data, options, targets) {
				...
				if(Type.isValue(data.attributes) ? data.attributes.length > 0 : false) {
					...
					for(var __ks_0 = 0, __ks_1 = data.attributes.length, attr; __ks_0 < __ks_1; ++__ks_0) {
						attr = data.attributes[__ks_0];
						var __ks_2;
						if((attr.declaration.kind === NodeKind.AttributeExpression) && (Type.isValue(__ks_2 = Attribute.get(attr.declaration, targets)) ? (attribute = __ks_2, true) : false)) {
							if(nc) {
			...
			__ks_sttc_get_0: function(data, targets) {
				...
				var clazz;
				if((Type.isValue($attributes[data.name.name]) ? (clazz = $attributes[data.name.name], true) : false) && ((clazz.target() & targets) > 0)) {
	...
	var IfAttribute = Helper.class({
		...
		__ks_func_evaluate_1: function(data, target) {
			...
			if(data.kind === NodeKind.AttributeExpression) {
				...
				var match;
				if(Type.isValue(__ks_0 = $semverRegex.exec(data.name)) ? (match = __ks_0, true) : false) {
					if(Type.isValue(match[2])) {
	...
	var Module = Helper.class({
		...
		__ks_func_path_0: function() {
			...
			var name = arguments[++__ks_i];
			if(!Type.isValue(x) || !Type.isValue(this._output)) {
				return name;
			}
			var output = null;
	...
	var $class = {
		abstractMethod: function(node, fragments, statement, signature, parameters, reflect, name) {
			...
			if(!Type.isArray(reflect.abstractMethods[name])) {
		...
		areAbstractMethodsImplemented: function(variable, parent, scope) {
			...
			if(Type.isValue(parent.extends)) {
		...
		continuous: function(node, fragments) {
			...
			if(node._es5) {
				...
				if(Type.isValue(node._data.version)) {
				...
				if(!node._extends || (Type.isValue(node._extendsVariable.sealed) ? node._extendsVariable.sealed.extern : false)) {
					clazz.newControl().code($class.methodHeader("$create", node), "()").step().line("this.__ks_init()").line("this.__ks_cons(arguments)");
				}
				if(noinit) {
					if(node._extends) {
						if(Type.isValue(node._extendsVariable.sealed) ? node._extendsVariable.sealed.extern : false) {
				...
				else if(Type.isValue(node._extendsVariable.sealed) ? node._extendsVariable.sealed.extern : false) {
		...
		hierarchy: function() {
			...
			var __ks_0;
			if(Type.isValue(variable.extends) && (Type.isValue(__ks_0 = node.scope().getVariable(variable.extends)) ? (variable = __ks_0, true) : false)) {
		...
		constructor: function(node, fragments, reflect) {
			...
			if(node._extends) {
				extend = function() {
					...
					var constructorName = (Type.isValue(node._extendsVariable.sealed) ? node._extendsVariable.sealed.extern : false) ? "constructor" : "__ks_cons";
			...
			if(methods.length === 0) {
				...
				if(min === Infinity) {
					...
					for(var i = min; i <= max; ++i) {
						var group;
						if(Type.isValue(groups[i]) ? (group = groups[i], true) : false) {
							var gg;
							for(var j = i + 1; (Type.isValue(groups[j]) ? (gg = groups[j], true) : false) && __ks_Array._cm_same(gg.methods, group.methods) && j <= max; ++j) {
								if(Type.isArray(group.n)) {
									group.n.push(j);
								}
								else {
									group.n = [i, j];
		...
		methodTypes: function(node, method, target, types) {
			...
			for(var p = 0, __ks_0 = method.parameters.length, parameter; p < __ks_0; ++p) {
				...
				for(var i = 1, __ks_1 = parameter.min; i <= __ks_1; ++i) {
					var type;
					if(Type.isValue(types[index]) ? (type = types[index], false) : true) {
						type = types[index] = {
							index: index,
							types: {}
						};
		...
		signature: function(data, node) {
			...
			for(var __ks_0 = 0, __ks_1 = data.parameters.length, parameter; __ks_0 < __ks_1; ++__ks_0) {
				...
				if(!last || !$method.sameType(type, last.type)) {
					...
					last = {
						type: type,
						min: (parameter.defaultValue || (Type.isValue(parameter.type) ? parameter.type.nullable : false)) ? 0 : 1,
						max: 1
					};
					...
					if(nf) {
						if(!(parameter.defaultValue || (Type.isValue(parameter.type) ? parameter.type.nullable : false))) {
							++last.min;
						}
						++last.max;
					}
			...
			if(Type.isValue(data.name)) {
				...
				for(var __ks_0 = 0, __ks_1 = data.modifiers.length, modifier; nf && __ks_0 < __ks_1; ++__ks_0) {
					...
					if(modifier.kind === ModifierKind.Alias) {
						var variable, __ks_2;
						if(Type.isValue(__ks_2 = node.getInstanceVariable(name)) ? (variable = __ks_2, true) : false) {
							if(Type.isValue(variable.type)) {
								return variable.type;
							}
						}
						else if(Type.isValue(__ks_2 = node.getInstanceVariable("_" + name)) ? (variable = __ks_2, true) : false) {
							if(Type.isValue(variable.type)) {
								return variable.type;
							}
						}
						else if(Type.isValue(__ks_2 = node.getInstanceMethod(name)) ? (variable = __ks_2, true) : false) {
							if(Type.isValue(variable.type)) {
								return variable.type;
							}
						}
	...
	var ClassDeclaration = Helper.class({
		...
		__ks_func_analyse_0: function() {
			...
			if(Type.isValue(data.extends)) {
				this._extends = true;
				var __ks_0;
				if(Type.isValue(__ks_0 = this._scope.getVariable(data.extends.name)) ? (this._extendsVariable = __ks_0, false) : true) {
				...
				if(Type.isValue(this._extendsVariable.sealed) ? this._extendsVariable.sealed.extern : false) {
					...
					if($method.isConstructor(member.name.name, this._variable)) {
						...
						if(instance) {
							if(method.statement.isAbstract()) {
								if(this._abstract) {
									if(!Type.isArray(this._abstractMethods[member.name.name])) {
		...
		__ks_func_getInstanceMethod_0: function() {
			...
			else if(Type.isValue(variable.extends)) {
		...
		__ks_func_toStatementFragments_0: function(fragments, mode) {
			...
			if(!this._es5 && Type.isValue(this._data.version)) {
				var line = fragments.newLine();
			...
			var references;
			if(Type.isValue(__ks_0 = this.module().listReferences(this._name)) ? (references = __ks_0, true) : false) {
		...
		define: function(declaration, node, kind) {
			...
			if(__ks_0 === NodeKind.ClassDeclaration) {
				...
				if(Type.isValue(declaration.extends)) {
					var superVar, __ks_1;
					if(Type.isValue(__ks_1 = node.scope().getVariable(declaration.extends.name)) ? (superVar = __ks_1, false) : true) {
	...
	var EnumDeclaration = Helper.class({
		...
		__ks_func_analyse_0: function() {
			...
			else if(Type.isValue(this._data.attributes)) {
				var nf = true;
	...
	var ForInStatement = Helper.class({
		...
		__ks_func_analyse_0: function() {
			...
			if(Type.isValue(this._data.value)) {
				if(!this._scope.hasVariable(this._data.value.name)) {
					var variable, __ks_0;
					if(Type.isValue(this._data.value.type)) {
						$variable.define(this, this._scope, this._data.value.name, $variable.kind(this._data.value.type), this._data.value.type);
					}
					else if((Type.isValue(__ks_0 = $variable.fromAST(this._data.expression, this)) ? (variable = __ks_0, true) : false) && ((Type.isValue(variable.type) && Type.isValue(variable.type.typeName)) ? variable.type.typeName.name === "Array" : false) && Type.isValue(variable.type.typeParameters)) {
			...
			if(Type.isValue(this._data.index)) {
			...
			if(!Type.isValue(this._index) && !(Type.isValue(this._data.index) && !this._data.declaration && this.greatScope().hasVariable(this._data.index.name))) {
			...
			if(Type.isValue(this._expressionName)) {
		...
		__ks_func_toStatementFragments_0: function(fragments, mode) {
			...
			var ctrl;
			if(this._data.desc) {
				if(Type.isValue(this._index) && !this._data.declaration && !this._defineIndex) {
					fragments.newLine().compile(this._index).code($equals).compile(Type.isValue(this._expressionName) ? this._expressionName : this._expression).code(".length - 1").done();
					ctrl = fragments.newControl().code("for(");
				}
				else {
					ctrl = fragments.newControl().code("for(", $variable.scope(this)).compile(Type.isValue(this._indexName) ? this._indexName : this._index).code($equals).compile(Type.isValue(this._expressionName) ? this._expressionName : this._expression).code(".length - 1");
				}
			}
			else {
				...
				ctrl.code(this._boundName, $equals).compile(Type.isValue(this._expressionName) ? this._expressionName : this._expression).code(".length");
			}
			if(this._data.declaration || this._defineValue) {
				ctrl.code($comma).compile(this._value);
			}
			ctrl.code("; ");
			if(Type.isValue(this._until)) {
				ctrl.code("!(").compile(this._until).code(") && ");
			}
			else if(Type.isValue(this._while)) {
			...
			if(Type.isValue(this._value)) {
				ctrl.newLine().compile(this._value).code($equals).compile(Type.isValue(this._expressionName) ? this._expressionName : this._expression).code("[").compile(Type.isValue(this._indexName) ? this._indexName : this._index).code("]").done();
			}
	...
	var ForOfStatement = Helper.class({
		...
		__ks_func_analyse_0: function() {
			...
			if(Type.isValue(this._data.key)) {
		...
		__ks_func_toStatementFragments_0: function(fragments, mode) {
			...
			ctrl.code(" in ").compile(Type.isValue(this._expressionName) ? this._expressionName : this._expression).code(")").step();
			if(Type.isValue(this._value)) {
				var line = ctrl.newLine();
				if(this._data.declaration || this._defineValue) {
					line.code($variable.scope(this));
				}
				line.compile(this._value).code($equals).compile(Type.isValue(this._expressionName) ? this._expressionName : this._expression).code("[").compile(Type.isValue(this._key) ? this._key : this._keyName).code("]").done();
			}
			if(Type.isValue(this._until)) {
	...
	var ForRangeStatement = Helper.class({
		...
		__ks_func_toStatementFragments_0: function(fragments, mode) {
			...
			ctrl.compile(this._value).code(" <= ").compile(Type.isValue(this._boundName) ? this._boundName : this._to).code("; ");
			if(this._data.by) {
				if(this._data.by.kind === NodeKind.NumericExpression) {
					...
					ctrl.compile(this._value).code(" += ").compile(Type.isValue(this._byName) ? this._byName : this._by);
				}
			}
			else {
				ctrl.code("++").compile(this._value);
	...
	var $function = {
		...
		useThisVariable: function(data, node) {
			...
			if(__ks_0 === NodeKind.ArrayExpression) {
				...
				if(Type.isValue(data.whenFalse) && data.$function.useThisVariable(data.whenFalse, node)) {
					return true;
				}
			}
			else if(__ks_0 === NodeKind.Literal) {
	...
	var Parameter = Helper.class({
		...
		__ks_func_analyse_0: function() {
			if(this._parent.isMethod()) {
				if(Type.isValue(this._data.name)) {
					var nf = true;
					...
					for(var __ks_0 = 0, __ks_1 = this._data.modifiers.length, modifier; nf && __ks_0 < __ks_1; ++__ks_0) {
						...
						if(modifier.kind === ModifierKind.Alias) {
							var variable, __ks_2;
							if(Type.isValue(__ks_2 = this._parent.getInstanceVariable(name)) ? (variable = __ks_2, true) : false) {
								if(Type.isValue(variable.type)) {
									this._type = $type.reference(variable.type);
								}
							}
							else if(Type.isValue(__ks_2 = this._parent.getInstanceVariable("_" + name)) ? (variable = __ks_2, true) : false) {
								if(Type.isValue(variable.type)) {
									this._type = $type.reference(variable.type);
								}
							}
							else if(Type.isValue(__ks_2 = this._parent.getInstanceMethod(name)) ? (variable = __ks_2, true) : false) {
								if(Type.isValue(variable.type)) {
									this._type = $type.reference(variable.type);
								}
							}
			...
			this._nullable = Type.isValue(this._type) ? this._type.nullable : undefined;
		},
		analyse: function() {
			if(arguments.length === 0) {
	...
	var IfStatement = Helper.class({
		...
		__ks_func_analyse_0: function() {
			...
			if(Type.isValue(this._data.whenFalse)) {
		...
		__ks_func_fuse_0: function() {
			this._condition.fuse();
			this._whenTrue.fuse();
			if(Type.isValue(this._whenFalse)) {
		...
		__ks_func_toIfFragments_0: function(fragments, mode) {
			...
			if(Type.isValue(this._whenFalse)) {
				if(Type.is(this._whenFalse, IfStatement)) {
	...
	var ImplementDeclaration = Helper.class({
		...
		__ks_func_analyse_0: function() {
			var __ks_0;
			if(Type.isValue(__ks_0 = this._scope.getVariable(this._data.variable.name)) ? (this._variable = __ks_0, false) : true) {
	...
	var ImplementClassMethodDeclaration = Helper.class({
		...
		__ks_func_toStatementFragments_0: function(fragments, mode) {
			...
			if(this._isContructor) {
				...
				var object = line.newObject();
				object.newLine().code("class: ", Type.isString(variable.name) ? variable.name : variable.name.name);
	...
	var ImplementClassMethodAliasDeclaration = Helper.class({
		...
		__ks_func_toStatementFragments_0: function(fragments, mode) {
			...
			var line = fragments.newLine().code($runtime.helper(this), ".", this._instance ? "newInstanceMethod" : "newClassMethod", "(");
			var object = line.newObject();
			object.line("class: ", Type.isString(variable.name) ? variable.name : variable.name.name);
			if(data.name.kind === NodeKind.TemplateExpression) {
	...
	var $import = {
		...
		loadDirectory: function() {
			...
			if(fs.isFile(pkgfile)) {
				...
				if(Type.isValue(pkg)) {
					var __ks_0, __ks_1;
					if(pkg.kaoscript && (Type.isValue(__ks_0 = $import.loadKSFile(path.join(x, pkg.kaoscript.main), moduleName, module, data, node)) ? (metadata = __ks_0, true) : false)) {
						return metadata;
					}
					else if(pkg.main && ((Type.isValue(__ks_0 = $import.loadFile(path.join(x, pkg.main), moduleName, module, data, node)) ? (metadata = __ks_0, true) : false) || (Type.isValue(__ks_1 = $import.loadDirectory(path.join(x, pkg.main), moduleName, module, data, node)) ? (metadata = __ks_1, true) : false))) {
						return metadata;
					}
				}
			}
		...
		loadNodeModule: function(x, start, module, data, node) {
			...
			for(var __ks_0 = 0, __ks_1 = dirs.length, dir; __ks_0 < __ks_1; ++__ks_0) {
				...
				var __ks_2, __ks_3;
				if((Type.isValue(__ks_2 = $import.loadFile(file, x, module, data, node)) ? (metadata = __ks_2, true) : false) || (Type.isValue(__ks_3 = $import.loadDirectory(file, x, module, data, node)) ? (metadata = __ks_3, true) : false)) {
					return metadata;
				}
			}
			return null;
		...
		resolve: function(data, y, module, node) {
			...
			if(/^(?:\.\.?(?:\/|$)|\/|([A-Za-z]:)?[\\\/])/.test(x)) {
				x = fs.resolve(y, x);
				var __ks_0, __ks_1;
				if((Type.isValue(__ks_0 = $import.loadFile(x, null, module, data, node)) ? (metadata = __ks_0, true) : false) || (Type.isValue(__ks_1 = $import.loadDirectory(x, null, module, data, node)) ? (metadata = __ks_1, true) : false)) {
					return metadata;
				}
			}
			else {
				if((Type.isValue(__ks_0 = $import.loadNodeModule(x, y, module, data, node)) ? (metadata = __ks_0, true) : false) || (Type.isValue(__ks_1 = $import.loadCoreModule(x, module, data, node)) ? (metadata = __ks_1, true) : false)) {
					return metadata;
				}
			}
	...
	var ThrowStatement = Helper.class({
		...
		__ks_func_analyse_0: function() {
			this._value = $compile.expression(this._data.value, this);
			var variable, __ks_0;
			if((Type.isValue(__ks_0 = $variable.fromAST(this._data.value, this)) ? (variable = __ks_0, true) : false) && variable.type && (Type.isValue(__ks_0 = $variable.fromType(variable.type, this)) ? (variable = __ks_0, true) : false)) {
	...
	var VariableDeclarator = Helper.class({
		...
		__ks_func_analyse_0: function() {
			...
			this._name = $compile.expression(this._data.name, this);
			if(Type.isValue(this._data.init)) {
				if(this._data.name.kind === NodeKind.Identifier) {
	...
	var ArrayRange = Helper.class({
		...
		__ks_func_analyse_0: function() {
			this._from = $compile.expression(Type.isValue(this._data.from) ? this._data.from : this._data.then, this);
			this._to = $compile.expression(Type.isValue(this._data.to) ? this._data.to : this._data.til, this);
			if(Type.isValue(this._data.by)) {
				this._by = $compile.expression(this._data.by, this);
			}
		},
		analyse: function() {
	...
	var ArrayComprehensionForFrom = Helper.class({
		...
		__ks_func_analyse_0: function() {
			...
			this._to = $compile.expression(Type.isValue(this._data.loop.to) ? this._data.loop.to : this._data.loop.til, this);
			if(Type.isValue(this._data.loop.by)) {
				this._by = $compile.expression(this._data.loop.by, this);
			}
			this._body = $compile.statement($return(this._data.body), this);
			this._body.analyse();
			if(Type.isValue(this._data.loop.when)) {
	...
	var ArrayComprehensionForIn = Helper.class({
		...
		__ks_func_analyse_0: function() {
			...
			if(Type.isValue(this._data.loop.value)) {
				var variable, __ks_0;
				if((Type.isValue(__ks_0 = $variable.fromAST(this._data.loop.expression, this)) ? (variable = __ks_0, true) : false) && ((Type.isValue(variable.type) && Type.isValue(variable.type.typeName)) ? variable.type.typeName.name === "Array" : false) && (Type.isValue(variable.type.typeParameters) ? variable.type.typeParameters.length === 1 : false)) {
			...
			if(Type.isValue(this._data.loop.index)) {
		...
		__ks_func_toFragments_0: function(fragments, mode) {
			...
			fragments.code(surround.beforeParameters).compile(Type.isValue(this._value) ? this._value : this._valueName);
			if(Type.isValue(this._index)) {
			...
			fragments.code(surround.footer);
			if(Type.isValue(this._when)) {
				fragments.code($comma).code(surround.beforeParameters).compile(Type.isValue(this._value) ? this._value : this._valueName);
				if(Type.isValue(this._index)) {
					fragments.code($comma).compile(this._index);
				}
	...
	var ArrayComprehensionForOf = Helper.class({
		...
		__ks_func_analyse_0: function() {
			this._expression = $compile.expression(this._data.loop.expression, this);
			if(Type.isValue(this._data.loop.key)) {
		...
		__ks_func_toFragments_0: function(fragments, mode) {
			...
			fragments.code(surround.beforeParameters).compile(Type.isValue(this._key) ? this._key : this._keyName);
			if(Type.isValue(this._value)) {
			...
			fragments.code(surround.footer);
			if(Type.isValue(this._when)) {
				fragments.code($comma).code(surround.beforeParameters).compile(Type.isValue(this._key) ? this._key : this._keyName);
				if(Type.isValue(this._value)) {
					fragments.code($comma).compile(this._value);
				}
				fragments.code(surround.afterParameters).newBlock().compile(this._when).done();
	...
	var CallExpression = Helper.class({
		...
		__ks_func_analyse_0: function() {
			...
			var variable;
			if((Type.isValue(__ks_0 = $call.variable(this._data, this)) ? (variable = __ks_0, true) : false) && (Type.isValue(variable.throws) ? variable.throws.length > 0 : false)) {
				for(var __ks_0 = 0, __ks_1 = variable.throws.length, name; __ks_0 < __ks_1; ++__ks_0) {
					name = variable.throws[__ks_0];
					var error, __ks_2;
					if(Type.isValue(__ks_2 = this._scope.getVariable(name)) ? (error = __ks_2, true) : false) {
	...
	var CreateExpression = Helper.class({
		...
		__ks_func_analyse_0: function() {
			var variable, __ks_0;
			if((Type.isValue(__ks_0 = $variable.fromAST(this._data.class, this)) ? (variable = __ks_0, true) : false) && variable.abstract) {
	...
	var Compiler = Helper.class({
		...
		__ks_func_compile_0: function(data) {
			...
			this._module = new Module(Type.isValue(data) ? data : fs.readFile(this._file), this, this._file);
