...
module.exports = function() {
				...
				if(list) {
					if(data.scope.kind === ScopeModifier.This) {
						if(data.callee.kind === Kind.Identifier) {
							variable = node.getVariable(data.callee.name);
						}
						if(Type.isValue(variable) && variable.callReplacement) {
				...
				if(clause.conditions.length) {
					...
					for(var i = 0, __ks_5 = __ks_4.length; i < __ks_5; ++i) {
						...
						if(condition.kind === Kind.SwitchConditionArray) {
							...
							var __ks_variable_1 = node.getVariable(data.expression.name);
							if(!__ks_variable_1 || (__ks_variable_1.type.kind !== VariableKind.Enum)) {
								throw new Error("Switch condition is not an Enum at line " + condition.start.line);
							}
							ctrl.code(name, " === ").compile(__ks_variable_1.type.name, config).code(".").compile(condition.name, config, Mode.Key);
						}
	...
	var $expression = {
		member(node, data, config, name) {
			...
			if(__ks_0 === Kind.BinaryOperator) {
				...
				return {
					name: [node.getRenamedVar(data.name)],
					operand: false
				};
			}
			else if(__ks_0 === Kind.MemberExpression) {
	...
	class Block {
		...
		__ks_func_codeVariable_0(data) {
			...
			var __ks_0;
			var name = Type.isValue((__ks_0 = this.getRenamedVar(data.name))) ? __ks_0 : data.name;
