		...
		assignment(node, data, config, mode) {
			...
			if(__ks_0 === AssignmentOperator.Addition) {
				...
				if(data.right.kind === Kind.Identifier) {
					if(mode & Mode.BooleanExpression) {
						node.code($runtime.type(config), ".isValue(").compile(data.right, config, Mode.Key).code(") ? (").compile(data.left, config, Mode.Key).code(" = ").compile(data.right, config, Mode.Assignment).code(", true) : false");
					}
					else {
						node.code($runtime.type(config), ".isValue(").compile(data.right, config, Mode.Key).code(") ? ").compile(data.left, config, Mode.Key).code(" = ").compile(data.right, config, Mode.Assignment).code(" : undefined");
					}
				}
				else {
					var name = node.newTempName();
					if(mode & Mode.BooleanExpression) {
						node.code($runtime.type(config), ".isValue(", name, " = ").compile(data.right, config, Mode.Key).code(") ? (").compile(data.left, config, Mode.Key).code(" = ", name, ", true) : false");
					}
					else {
						node.code($runtime.type(config), ".isValue(", name, " = ").compile(data.right, config, Mode.Key).code(") ? ").compile(data.left, config, Mode.Key).code(" = ", name, " : undefined");
					}
				}
			}
		...
		binary(node, data, config, mode) {
			...
			if(__ks_0 === BinaryOperator.And) {
				...
				if(data.left.kind === data.right.kind && data.right.kind === Kind.Identifier) {
					node.code($runtime.type(config), ".isValue(").compile(data.left, config, Mode.Operand).code(") ? ").compile(data.left, config, Mode.Operand).code(" : ").compile(data.right, config, Mode.Operand);
				}
				else {
					node.code($runtime.type(config), ".vexists(").compile(data.left, config, Mode.Operand).code(", ").compile(data.right, config, Mode.Operand).code(")");
				}
			}
