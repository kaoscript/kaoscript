	...
	foobar.__ks_rt = function(that, args) {
		...
		if(args.length >= 1 && args.length <= 3) {
			if(Helper.isVarargs(args, 1, 3, t0, pts = [0], 0)) {
				if(Helper.isVarargs(args, 0, 1, t1, pts, 1) && Helper.isVarargs(args, 0, 2, t2, pts, 2) && te(pts, 3)) {
			...
			if(Helper.isVarargs(args, 1, 3, t2, pts = [0], 0) && te(pts, 1)) {
				return foobar.__ks_0.call(that, Helper.getVarargs(args, 0, pts[1]));
			}
			throw Helper.badArgs();
		}
		if(args.length >= 4 && args.length <= 7) {
			if(Helper.isVarargs(args, 1, 3, t0, pts = [0], 0) && Helper.isVarargs(args, 0, 1, t1, pts, 1) && Helper.isVarargs(args, 0, 3, t2, pts, 2) && te(pts, 3)) {
				return foobar.__ks_1.call(that, Helper.getVarargs(args, 0, pts[1]), Helper.getVararg(args, pts[1], pts[2]), Helper.getVarargs(args, pts[2], pts[3]));
			}
		}
		throw Helper.badArgs();
