	...
	foobar.__ks_rt = function(that, args) {
		...
		if(args.length === 2) {
			...
			if(t1(args[0]) && t0(args[1])) {
				return foobar.__ks_0.call(that, [args[0]], args[1], void 0, void 0);
			}
			throw Helper.badArgs();
		}
		if(args.length >= 3) {
			if(Helper.isVarargs(args, 0, args.length - 3, t1, pts = [0], 0)) {
				if(Helper.isVarargs(args, 1, 1, t0, pts, 1)) {
					if(Helper.isVarargs(args, 1, 1, t2, pts, 2) && Helper.isVarargs(args, 1, 1, t3, pts, 3) && te(pts, 4)) {
				...
				if(Helper.isVarargs(args, 1, 1, t1, pts, 1)) {
					if(Helper.isVarargs(args, 1, 1, t0, pts, 2)) {
						if(Helper.isVarargs(args, 1, 1, t2, pts, 3) && te(pts, 4)) {
							return foobar.__ks_0.call(that, Helper.getVarargs(args, 0, pts[2]), Helper.getVararg(args, pts[2], pts[3]), Helper.getVararg(args, pts[3], pts[4]), void 0);
						}
					}
					if(Helper.isVarargs(args, 1, 1, t1, pts, 2) && Helper.isVarargs(args, 1, 1, t0, pts, 3) && te(pts, 4)) {
						return foobar.__ks_0.call(that, Helper.getVarargs(args, 0, pts[3]), Helper.getVararg(args, pts[3], pts[4]), void 0, void 0);
					}
					throw Helper.badArgs();
				}
				throw Helper.badArgs();
