...
module.exports = function() {
	...
	f.__ks_0 = function(numbers, flag, strings) {
		return Helper.toString(foobar(...numbers, flag, ...strings));
	};
	f.__ks_rt = function(that, args) {
		const t0 = Type.isNumber;
		const t1 = Type.isBoolean;
		const t2 = value => Type.isArray(value, Type.isString);
		const te = (pts, idx) => Helper.isUsingAllArgs(args, pts, idx);
		let pts;
		if(args.length >= 2) {
			if(Helper.isVarargs(args, 0, args.length - 2, t0, pts = [0], 0) && Helper.isVarargs(args, 1, 1, t1, pts, 1) && Helper.isVarargs(args, 1, 1, t2, pts, 2) && te(pts, 3)) {
				return f.__ks_0.call(that, Helper.getVarargs(args, 0, pts[1]), Helper.getVararg(args, pts[1], pts[2]), Helper.getVararg(args, pts[2], pts[3]));
			}
		}
		throw Helper.badArgs();
	};
