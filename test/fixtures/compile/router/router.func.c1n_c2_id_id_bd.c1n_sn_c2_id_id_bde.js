...
module.exports = function() {
	...
	foobar.__ks_rt = function(that, args) {
		...
		if(args.length === 3) {
			if(t0(args[0])) {
				if(t1(args[1])) {
					if(Helper.isVarargs(args, 0, 1, t2, pts = [2], 0) && Helper.isVarargs(args, 0, 1, t3, pts, 1) && te(pts, 2)) {
						return foobar.__ks_0.call(that, args[0], args[1], Helper.getVararg(args, 2, pts[1]), void 0, Helper.getVararg(args, pts[1], pts[2]));
					}
					throw Helper.badArgs();
				}
			...
			if(t0(args[0])) {
				if(t1(args[1])) {
					if(t2(args[2])) {
						...
						if(t2(args[3])) {
							return foobar.__ks_0.call(that, args[0], args[1], args[2], args[3], void 0);
						}
						throw Helper.badArgs();
					}
					throw Helper.badArgs();
				}
				if(t4(args[1]) && t1(args[2]) && Helper.isVarargs(args, 0, 1, t2, pts = [3], 0) && Helper.isVarargs(args, 0, 1, t3, pts, 1) && te(pts, 2)) {
					return foobar.__ks_1.call(that, args[0], args[1], args[2], Helper.getVararg(args, 3, pts[1]), void 0, Helper.getVararg(args, pts[1], pts[2]));
				}
				throw Helper.badArgs();
			}
			throw Helper.badArgs();
			...
			if(t0(args[0])) {
				if(t4(args[1])) {
					if(t1(args[2]) && t2(args[3])) {
						if(t3(args[4])) {
							return foobar.__ks_1.call(that, args[0], args[1], args[2], args[3], void 0, args[4]);
						}
						if(t2(args[4])) {
