...
module.exports = function() {
	...
	foobar.__ks_rt = function(that, args) {
		...
		const t2 = value => Type.isClassInstance(value, Quxbaz);
		const t3 = value => Type.isClassInstance(value, Foobar);
		const t4 = value => Type.isString(value) || Type.isNull(value);
		if(args.length === 4) {
			if(t0(args[0])) {
				if(t1(args[1]) && t1(args[2]) && t2(args[3])) {
					return foobar.__ks_1.call(that, args[0], args[1], args[2], args[3]);
				}
			}
			if(t1(args[0]) && t1(args[1]) && t3(args[2]) && t2(args[3])) {
				return foobar.__ks_0.call(that, args[0], args[1], args[2], args[3]);
			}
			throw Helper.badArgs();
		}
		if(args.length === 5) {
			if(t4(args[0]) && t1(args[1]) && t1(args[2]) && t3(args[3]) && t2(args[4])) {
