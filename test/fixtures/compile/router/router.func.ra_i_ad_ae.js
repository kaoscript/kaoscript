		...
		if(args.length >= 3) {
			if(Helper.isVarargs(args, 0, args.length - 3, t1, pts = [0], 0)) {
				if(Helper.isVarargs(args, 1, 1, t0, pts, 1)) {
					if(Helper.isVarargs(args, 1, 1, t2, pts, 2) && Helper.isVarargs(args, 1, 1, t1, pts, 3) && te(pts, 4)) {
				...
				if(Helper.isVarargs(args, 1, 1, t1, pts, 1) && Helper.isVarargs(args, 1, 1, t0, pts, 2) && Helper.isVarargs(args, 1, 1, t1, pts, 3) && te(pts, 4)) {
