...
module.exports = function() {
	...
	foobar.__ks_rt = function(that, args) {
		...
		if(args.length === 5) {
			if(t0(args[0])) {
				if(t0(args[1])) {
					...
					if(t1(args[3]) && t1(args[4])) {
						return foobar.__ks_0.call(that, [args[0], args[1]], args[2], [args[3], args[4]]);
					}
				}
				if(t1(args[2]) && t1(args[3]) && t1(args[4])) {
					return foobar.__ks_0.call(that, [args[0]], args[1], [args[2], args[3], args[4]]);
				}
				throw Helper.badArgs();
			}
		...
		if(args.length === 7) {
			if(t0(args[0]) && t0(args[1]) && t0(args[2]) && t1(args[4]) && t1(args[5]) && t1(args[6])) {
