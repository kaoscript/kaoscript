...
module.exports = function() {
	...
	function foobar() {
		...
		if(test(x = "1") && test(y = x) && test(x = 3)) {
			console.log(__ks_Number._im_toInt(x));
			console.log(__ks_String._im_toInt(y));
		}
		else {
			console.log(Type.isString(x) ? __ks_String._im_toInt(x) : __ks_Number._im_toInt(x));
			console.log(Type.isBoolean(y) ? __ks_Boolean._im_toInt(y) : __ks_String._im_toInt(y));
		}
		console.log(Type.isNumber(x) ? __ks_Number._im_toInt(x) : __ks_String._im_toInt(x));
		console.log(Type.isString(y) ? __ks_String._im_toInt(y) : __ks_Boolean._im_toInt(y));
	}
};