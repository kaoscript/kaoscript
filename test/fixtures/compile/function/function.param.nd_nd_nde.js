		...
		const __ks_rt = (...args) => {
			...
			if(args.length <= 3) {
				if(Helper.isVarargs(args, 0, 1, t0, pts = [0], 0) && Helper.isVarargs(args, 0, 1, t0, pts, 1) && Helper.isVarargs(args, 0, 1, t0, pts, 2) && te(pts, 3)) {
					return __ks_rt.__ks_0.call(null, Helper.getVararg(args, 0, pts[1]), Helper.getVararg(args, pts[1], pts[2]), Helper.getVararg(args, pts[2], pts[3]));
				}
			}
			throw Helper.badArgs();
		};
		__ks_rt.__ks_0 = function(x = null, y = null, z = null) {
			return [x, y, z];
		};
		return __ks_rt;
	})();
