...
module.exports = function() {
	...
	formatToHex.__ks_rt = function(that, args) {
		...
		if(args.length === 1) {
			if(t0(args[0])) {
				return formatToHex.__ks_0.call(that, args[0]);
			}
		}
		throw Helper.badArgs();
	};
	function formatToSRGB() {
		return formatToSRGB.__ks_rt(this, arguments);
	};
	formatToSRGB.__ks_0 = function(that) {
		if(that._alpha === 1) {
	...
	formatToSRGB.__ks_rt = function(that, args) {
		const t0 = value => Type.isClassInstance(value, Color);
		if(args.length === 1) {
			if(t0(args[0])) {
				return formatToSRGB.__ks_0.call(that, args[0]);
			}
		}
		throw Helper.badArgs();
	};
