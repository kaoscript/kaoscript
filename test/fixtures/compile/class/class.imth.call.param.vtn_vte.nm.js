...
module.exports = function() {
		...
		__ks_func_foo_rt(that, proto, args) {
			...
			if(args.length >= 1 && args.length <= 2) {
				if(Helper.isVarargs(args, 0, 1, t0, pts = [0], 0) && Helper.isVarargs(args, 1, 1, t1, pts, 1) && te(pts, 2)) {
					return proto.__ks_func_foo_0.call(that, Helper.getVararg(args, 0, pts[1]), Helper.getVararg(args, pts[1], pts[2]));
				}
			}
			throw Helper.badArgs();
		}
	...
	console.log(x.__ks_func_foo_0(1, "foo"));
};