...
module.exports = function() {
	...
	class Greetings {
		...
		__ks_func_greet_rt(that, proto, args) {
			...
			const t1 = value => Type.isDexObject(value, 1, Type.isString) || Type.isNull(value);
			const t2 = Type.isString;
			if(args.length === 4) {
				if(t0(args[0]) && t1(args[1]) && t1(args[2]) && t2(args[3])) {
					return proto.__ks_func_greet_0.call(that, args[0], args[1], args[2], args[3]);
				}
			}
			throw Helper.badArgs();
		}
	...
	class MyGreetings extends Greetings {
		...
		__ks_func_greet_0(person, school, district, message) {
			if(Type.isClassInstance(person, Student) && (Type.isDexObject(school, 1, Type.isString) || Type.isNull(school)) && (Type.isDexObject(district, 1, Type.isString) || Type.isNull(district)) && Type.isString(message)) {
		...
		__ks_func_greet_rt(that, proto, args) {
			...
			if(args.length === 4) {
				if(t0(args[0]) && t1(args[1]) && t1(args[2]) && t2(args[3])) {
					return proto.__ks_func_greet_1.call(that, args[0], args[1], args[2], args[3]);
				}
			}
