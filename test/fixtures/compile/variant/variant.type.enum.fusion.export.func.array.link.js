	...
	const __ksType = {
		...
		isSchoolPerson: (value, cast, filter) => __ksType.isRange(value) && Type.isDexObject(value, 1, 0, {kind: variant => {
			...
			if(variant === PersonKind.Teacher) {
				return Type.isDexObject(value, 0, 0, {favorite: value => __ksType.isSchoolPerson(value, cast, value => value === PersonKind.Student), cards: value => Type.isArray(value, value => __ksType.isCard(value, cast, CardSuit.__ks_eq_Reds))});
			}
			return true;
		}}),
