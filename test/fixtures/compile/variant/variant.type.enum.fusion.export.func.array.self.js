...
module.exports = function() {
	...
	const SchoolPerson = Helper.alias((value, cast, filter) => Position.is(value) && Type.isDexObject(value, 1, 0, {kind: variant => {
		if(cast) {
		...
		if(variant === PersonKind.Teacher) {
			return Type.isDexObject(value, 0, 0, {favorites: value => Type.isArray(value, value => SchoolPerson.is(value, cast, value => value === PersonKind.Student))});
		}
		return true;
	}}));
	function greeting() {
