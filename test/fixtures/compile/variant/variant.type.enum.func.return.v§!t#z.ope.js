...
module.exports = function() {
	const __ksType = {
		isWorkingDay: value => value === Weekday.MONDAY || value === Weekday.TUESDAY || value === Weekday.WEDNESDAY || value === Weekday.THURSDAY || value === Weekday.FRIDAY || value === Weekday.SATURDAY,
		isWorkingDayData: (value, cast, filter) => Type.isDexObject(value, 1, 0, {kind: variant => {
			if(cast) {
				if((variant = Weekday(variant)) === null || !__ksType.isWorkingDay(variant)) {
					return false;
				}
				value["kind"] = variant;
			}
			else if(!__ksType.isWorkingDay(variant)) {
				return false;
			}
