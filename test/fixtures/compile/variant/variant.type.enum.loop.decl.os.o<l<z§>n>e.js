	...
	const __ksType = {
		isSchoolPerson: (value, cast, filter) => Type.isDexObject(value, 1, 0, {kind: variant => {
			...
			if(variant === PersonKind.Teacher) {
				return Type.isDexObject(value, 0, 0, {students: value => Type.isArray(value, value => __ksType.isSchoolPerson(value, cast, value => value === PersonKind.Student)) || Type.isNull(value)});
			}
			return true;
		}})
	};
