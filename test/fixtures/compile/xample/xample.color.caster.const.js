...
module.exports = function() {
	...
	function srgb(that, color) {
		...
		if(Type.isValue(match)) {
			that._red = Math.round(Operator.multiplication(2.55, $caster.percentage(match[1])));
			that._green = Math.round(Operator.multiplication(2.55, $caster.percentage(match[2])));
			that._blue = Math.round(Operator.multiplication(2.55, $caster.percentage(match[3])));
			return true;
		}
		else {
			return false;
